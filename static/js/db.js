...
export default class DB {
  ...
  async init() {
    this.db = await idb.openDB(this.dbName, 1, {
      upgrade(db) {
        let store = db.createObjectStore("default", {
          keyPath: "id",
          autoIncrement: true,
        });
      },
  ...
  async add(input) {
    return await this.db.add("default", input);
  }

  async get(id) {
    return await this.db.get("default", id);
  }

  async set(id, input) {
    if (typeof id === "string") id = parseInt(id);
    return await this.db.put("default", input);
  }

  async remove(id) {
    if (typeof id === "string") id = parseInt(id);
    return await this.db.delete("default", id);
  }

  async list() {
    let output = {};
    let cursor = await this.db.transaction("default").store.openCursor();
    while (cursor) {
      output[cursor.value.id] =
        cursor.value["$id"].replace(".json", "") || "Unnamed Schema";
